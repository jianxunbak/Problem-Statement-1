...
class RevitWorkers:
    ...
    def execute_fast_manifest(self, manifest):
        ...
        try:
            ...
            self._process_walls(current_levels, elevations, floor_dims, registry, results, created, reused, affected_elements)
    ...
    def _process_levels(self, manifest, registry, created, reused):
        ...
        # 3. ROBUSTNESS: Ensure ALL existing AI levels are preserved 
        # even if not in the current elevations list (prevents culling on partial updates).
        for tag, eid in registry.items():
            if tag.startswith("AI_Level_") and tag not in [f"AI_Level_{i+1}" for i in range(len(elevations))]:
    ...
    def _process_floors(self, current_levels, floor_dims, shell, registry, results, created, reused, affected_elements):
        ...
        for k, lvl in enumerate(current_levels):
            ...
            # Combine lift voids (all floors) + stair voids (skip 1st floor)
            shaft_voids = getattr(self, '_shaft_voids', None) or []
            stair_voids = getattr(self, '_stair_voids', None) or []
            # Staircase voids only from 2nd floor up (k > 0) — no void
            # on ground floor when there is no basement below.
            all_voids = list(shaft_voids) + (list(stair_voids) if k > 0 else [])
            # Slab half-extents for containment check
            slab_hx = w_ft / 2.0
            slab_hy = l_ft / 2.0
            if all_voids:
                for (vx1, vy1, vx2, vy2) in all_voids:
                    # Skip void if it extends outside the slab boundary —
                    # otherwise Floor.Create fails with invalid curve loops.
                    if vx1 < -slab_hx or vx2 > slab_hx or vy1 < -slab_hy or vy2 > slab_hy:
                        continue
                    # Inner void loop — clockwise (opposite winding to outer CCW boundary)
    ...
    def _process_columns_and_grids(self, current_levels, elevations, floor_dims, expanded_slab_dims, shell, registry, results, created, reused, affected_elements=[], core_bounds=None):
        ...
        # 2. GRID LOGIC — uniform grid from center + edge columns at offset_from_edge.
        # Core is structural so no columns are placed inside its footprint.
        # Edge columns are ALWAYS placed at slab_edge - offset_from_edge.
        def get_grid_offsets(dim_mm, span_ft):
            half_dim_ft = mm_to_ft(dim_mm) / 2.0
            limit_ft = half_dim_ft - offset_from_edge_ft
            offsets = set()
            # Always include edge columns at offset_from_edge from slab edge
            offsets.add(round(limit_ft, 4))
            offsets.add(round(-limit_ft, 4))
            # Regular grid from center
            curr = 0.0
            while curr <= limit_ft + 0.01:
                offsets.add(round(curr, 4))
                curr += span_ft
            curr = -span_ft
            while curr >= -limit_ft - 0.01:
                offsets.add(round(curr, 4))
        ...
        x_offsets = get_grid_offsets(max_w, span_ft)
        y_offsets = get_grid_offsets(max_l, span_ft)
        ...
        if center_only:
            ...
            x_offsets = [o for o in x_offsets if abs(o) < half_w_ft - 0.1]
            y_offsets = [o for o in y_offsets if abs(o) < half_l_ft - 0.1]
        ...
        for ox in x_offsets:
            for oy in y_offsets:
                ...
                if core_bounds:
                    if (core_bounds[0] < ox < core_bounds[2]) and \
                       (core_bounds[1] < oy < core_bounds[3]):
                        continue
        ...
        for (ix, iy, ox, oy), k_max in max_level_for_grid.items():
            for k in range(k_max):
                count += 1
                if count % 100 == 0:
                    self.log("Step 3c Progress: Created/Updated {}/{} columns...".format(count, total_cols))
    ...
    def _expand_lifts_in_manifest(self, manifest, current_levels, elevations, floor_dims, affected_elements=[], results=None):
        ...
        if is_expansion:
            ...
            # FLOOR-PLATE EDIT: lock existing count to prevent geometry change
            if existing_lift_count is not None:
                num_lifts_val = existing_lift_count
            elif num_lifts_raw and num_lifts_raw not in ["random", "auto", "calculated"]:
                num_lifts_val = int(safe_num(num_lifts_raw, 2))
            else:
                num_lifts_val = int(safe_num(
                    lift_logic.calculate_lift_requirements(num_storeys, avg_h, total_occ), 2))
        ...
        if not new_levels_data:
            ...
            # 3. Compute shaft voids (geometry hasn't changed, just heights)
            shaft_voids_ft = []
            remaining_h = num_lifts_val
            for b_idx in range(layout['num_blocks']):
                b_lifts = min(remaining_h, layout['lifts_per_block'])
                remaining_h -= b_lifts
                b_y_offset = lift_logic.get_block_y_offset(b_idx, layout['num_blocks'], layout['block_d'])
                b_center = [center_pos[0], center_pos[1] + b_y_offset]
        ...
        for b_idx in range(layout['num_blocks']):
            ...
            # Per-shaft void rectangles for floor slab openings
            for (x1, y1, x2, y2) in lift_logic.get_shaft_void_rectangles_mm(
                    b_lifts, b_center_pos, lift_size, lobby_w):
                shaft_voids_ft.append((mm_to_ft(x1), mm_to_ft(y1), mm_to_ft(x2), mm_to_ft(y2)))
        ...
        if all_wall_bounds_xs:
            return (min(all_wall_bounds_xs), min(all_wall_bounds_ys),
    ...
    def _expand_staircases_in_manifest(self, manifest, current_levels, elevations,
        ...
        # --- 13. Update core_bounds to include staircase footprint [rule g] ---
        if stair_walls:
    ...
    def _create_stair_runs(self, current_levels, results):
        """Create dogleg staircases using Revit's native StairsEditScope.
        ...
        class _StairFailHandler(DB.IFailuresPreprocessor):
            __namespace__ = "StairFH_" + str(id(object()))
        ...
        landing_mm = spec.get("landing_width", 1800)
        spec_tread_mm = spec.get("tread", 275)
        ...
        for rd in run_data_list:
            ...
            try:
                ...
                # Main landing Y bounds (for intermediate landings)
                ml_y_start_ft = mm_to_ft(main_land.get('y_start', flight_y_start_mm))
                ml_y_end_ft = mm_to_ft(main_land.get('y_end', flight_y_start_mm))
                ml_x_left = mm_to_ft(main_land.get('x_left', f1_cx_mm - width_mm / 2.0))
                ml_x_right = mm_to_ft(main_land.get('x_right', f2_cx_mm + width_mm / 2.0))
                ...
                # --- Build runs + explicit landings per pair ---
                ...
                for p_idx, (a_risers, b_risers) in enumerate(flight_pairs):
                    ...
                    # --- Intermediate landing (between pairs) ---
                    if p_idx > 0:
                        lm1 = DB.XYZ(ml_x_left,  ml_y_start_ft, current_elev_abs)
                        lm2 = DB.XYZ(ml_x_right, ml_y_start_ft, current_elev_abs)
                        lm3 = DB.XYZ(ml_x_right, ml_y_end_ft,   current_elev_abs)
                        lm4 = DB.XYZ(ml_x_left,  ml_y_end_ft,   current_elev_abs)
                        ml_loop = DB.CurveLoop()
                        ml_loop.Append(DB.Line.CreateBound(lm1, lm2))
                        ml_loop.Append(DB.Line.CreateBound(lm2, lm3))
                        ml_loop.Append(DB.Line.CreateBound(lm3, lm4))
                        ml_loop.Append(DB.Line.CreateBound(lm4, lm1))
                        _StairsLanding.CreateSketchedLanding(
                            doc, stair_id, ml_loop, current_elev_rel)
                    ...
                    if b_risers <= 0:
                        # Odd total — only flight A in this pair
                        current_elev_rel = mid_elev_rel
                        self.log("  Pair {}/{}: A={} run={:.0f}mm "
                                 "z={:.0f}mm (no B)".format(
                            p_idx + 1, len(flight_pairs), a_risers,
                            a_run_len * 304.8, mid_elev_abs * 304.8))
                        continue
                ...
                scope.Commit(_StairFailHandler())
                scope = None
                succeeded = True
